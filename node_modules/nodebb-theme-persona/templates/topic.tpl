					...
					<div class="posts-container" style="min-width: 0;">
						...
						{{{ each posts }}}
							<li component="post" class="pt-4 {{{ if posts.deleted }}}deleted{{{ end }}} {{{ if posts.selfPost }}}self-post{{{ end }}} {{{ if posts.topicOwnerPost }}}topic-owner-post{{{ end }}}" <!-- IMPORT partials/data/topic.tpl -->>
								...
								<!-- Reaction Buttons and Counts - Change Implemented Here -->
								<div class="post-actions">
								    <!-- Existing Reply Button -->
								    <button component="post/reply" class="btn btn-link">Reply</button>

								    <!-- Emoji Reactions with Counts -->
								    <div class="emoji-reactions" data-pid="{posts.pid}">
								        <button class="reaction" data-reaction="👍">👍 (<span id="count-👍-{posts.pid}">0</span>)</button>
								        <button class="reaction" data-reaction="❤️">❤️ (<span id="count-❤️-{posts.pid}">0</span>)</button>
								        <button class="reaction" data-reaction="😂">😂 (<span id="count-😂-{posts.pid}">0</span>)</button>
								    </div>
								</div>
								<!-- End of Change -->
...
<script>
    $(document).ready(function() {
        // Fetch initial reaction counts when the page loads
        $('.emoji-reactions').each(function() {
            var postId = $(this).data('pid');  // Get the post ID for each emoji-reactions div

            // Make a GET request to fetch the reaction counts from the server
            $.get('/api/post/' + postId + '/reactions', function(response) {
                if (response.success) {
                    // Iterate through the reactions object and update the counts
                    Object.keys(response.reactions).forEach(function(reaction) {
                        $('#count-' + reaction + '-' + postId).text(response.reactions[reaction]);
                    });
                }
            });
        });
        ...
        $('.reaction').on('click', function() {
            var reaction = $(this).data('reaction');    // The emoji clicked
            var postId = $(this).closest('.emoji-reactions').data('pid');    // The post ID

            // Make an AJAX POST request to send the reaction to the server
            $.post('/api/post/' + postId + '/reaction', { reaction: reaction }, function(response) {
                if (response.success) {
                    // Update the emoji reaction count on success
