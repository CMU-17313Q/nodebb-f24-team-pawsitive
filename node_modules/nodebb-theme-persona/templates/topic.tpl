...
<!-- Client-side JavaScript for handling reactions -->
<script type="text/javascript">
    $(document).ready(function() {
        // Handle click events on the reaction buttons
        $('.reaction').on('click', function() {
            var reaction = $(this).data('reaction');  // Get the reaction type
            var postId = $(this).data('pid');          // Get the post ID

            // Send the reaction data to the server via an AJAX POST request
            $.post('/api/post/' + postId + '/reaction', { reaction: reaction }, function(response) {
                if (response.success) {
                    var countElem = $('#count-' + reaction + '-' + postId);
                    countElem.text(parseInt(countElem.text()) + 1);  // Increment the count
                } else {
