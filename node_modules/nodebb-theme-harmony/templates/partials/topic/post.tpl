...
<div component="post/footer" class="post-footer border-bottom pb-2">
	...
	<div component="post/actions" class="d-flex justify-content-end gap-1 post-tools">
		...
		<!-- New Mark as Solved Button -->
		<a component="post/mark-solved" href="#" class="btn-ghost-sm solved-button" title="Mark as Solved" data-solved="false">
			<!-- Default unsolved state (red) -->
			<i class="fa fa-fw fa-square text-danger"></i>
			<span class="solved-text text-danger">Mark As Solved</span>
		</a>
...
<script>
	// Toggle solved state when the button is clicked
	document.addEventListener('DOMContentLoaded', function() {
		const solvedButton = document.querySelector('.solved-button');
		if (solvedButton) {
			solvedButton.addEventListener('click', function(e) {
				e.preventDefault();
				const isSolved = solvedButton.getAttribute('data-solved') === 'true';

				// Toggle the state, icon, and text
				if (isSolved) {
					solvedButton.setAttribute('data-solved', 'false');
					solvedButton.querySelector('i').classList.remove('fa-check', 'text-success');
					solvedButton.querySelector('i').classList.add('fa-square', 'text-danger');
					solvedButton.querySelector('.solved-text').classList.remove('text-success');
					solvedButton.querySelector('.solved-text').classList.add('text-danger');
					solvedButton.querySelector('.solved-text').textContent = 'Mark As Solved';
				} else {
					solvedButton.setAttribute('data-solved', 'true');
					solvedButton.querySelector('i').classList.remove('fa-square', 'text-danger');
					solvedButton.querySelector('i').classList.add('fa-check', 'text-success');
					solvedButton.querySelector('.solved-text').classList.remove('text-danger');
					solvedButton.querySelector('.solved-text').classList.add('text-success');
					solvedButton.querySelector('.solved-text').textContent = 'Marked As Solved';
				}
			});
		}
	});
