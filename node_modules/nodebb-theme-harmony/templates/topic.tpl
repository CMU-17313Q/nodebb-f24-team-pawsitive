                            ...
                            <li component="post" class="pt-4 {{{ if posts.deleted }}}deleted{{{ end }}} {{{ if posts.selfPost }}}self-post{{{ end }}} {{{ if posts.topicOwnerPost }}}topic-owner-post{{{ end }}}" <!-- IMPORT partials/data/topic.tpl -->>
                                ...
                                <div class="post-actions d-flex align-items-center gap-2">
                                    ...
                                    <div class="emoji-reactions" data-pid="{posts.pid}">
                                        <button class="reaction btn btn-sm btn-outline-secondary" data-reaction="👍">👍 (<span id="count-👍-{posts.pid}">0</span>)</button>
                                        <button class="reaction btn btn-sm btn-outline-secondary" data-reaction="❤️">❤️ (<span id="count-❤️-{posts.pid}">0</span>)</button>
                                        <button class="reaction btn btn-sm btn-outline-secondary" data-reaction="😂">😂 (<span id="count-😂-{posts.pid}">0</span>)</button>
...
<!-- JavaScript Section to Handle Emoji Reactions -->
<script>
    (function() {
        function setupEmojiReactions() {
            ...
            $(document).on('click', '.reaction', function() {
                var reaction = $(this).data('reaction');    // Emoji clicked
                var postId = $(this).closest('.emoji-reactions').data('pid');    // Post ID

                // Send reaction data via AJAX POST
                $.post('/api/post/' + postId + '/reaction', { reaction: reaction })
            ...
            $('.emoji-reactions').each(function() {
                ...
                $.get('/api/post/' + postId + '/reactions')
                  ...
                  .fail(function(jqXHR) {
                      console.error('Failed to fetch reactions:', jqXHR.statusText);
                  });
            });
        }

        // Run setupEmojiReactions when content is loaded via AJAX
