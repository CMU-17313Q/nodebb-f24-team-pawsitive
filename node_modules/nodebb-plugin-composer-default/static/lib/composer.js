...
define('composer', [
	...
	composer.enhance = function (postContainer, post_uuid, postData) {
		...
		*/
		...
		//adding click for anon function
		postContainer.on('click', '.composer-submit-anonymously', function (e) {
			e.preventDefault();
			e.stopPropagation();
		
			$(this).attr('disabled', true);
			composer.posts[post_uuid].isAnonymous = true; // Set the anonymous flag
			post(post_uuid); // Call the post function
		});

		
		// Mouse trap binding for "mod + Enter"
		require(['mousetrap'], function (mousetrap) {
			mousetrap(postContainer.get(0)).bind('mod+enter', function () {
				postContainer.find('.composer-submit-anonymously').attr('disabled', true);
				composer.posts[post_uuid].isAnonymous = true; // Set the flag
				post(post_uuid); // Call the post function
			});
		});
	...
	async function post(post_uuid) {
		...
		const submitBtn = postContainer.find('.composer-submit');
			...
			// Check the state of the Post Anonymously checkbox
			const isAnonymous = postContainer.find('#postAnonCheckbox').is(':checked');
			composer.posts[post_uuid].isAnonymous = isAnonymous; // Set the anonymous flag
		...
		if (action === 'topics.post') {
			...
			composerData = {
				...
				isAnonymous: composer.posts[post_uuid].isAnonymous, // adding isAnon flag
			};
