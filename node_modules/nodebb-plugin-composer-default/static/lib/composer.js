...
define('composer', [
	...
	composer.enhance = function (postContainer, post_uuid, postData) {
		...
		*/
		...
		postContainer.find('.composer-submit').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation(); // Prevents composer from being activated again
			
			$(this).attr('disabled', true);
			const isAnonymous = $(this).data('action') === 'submit-anonymously'; // Check if it's the anonymous submit
			post(post_uuid, isAnonymous); // Pass isAnonymous flag to the post function
		});
